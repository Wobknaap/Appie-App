...
def fetch_etten_leur_stores(token):
    ...
    query = """
    query FindStores {
      storesSearch(filter: {cityStartsWith: "Etten-Leur"}, limit: 50) {
        result {
          id
          name
          address {
    ...
    print("Fetching stores for Etten-Leur...")
...
if __name__ == "__main__":
    token = get_ah_token()
    if token:
        new_stores = fetch_etten_leur_stores(token)
        if new_stores:
            ...
            print(f"Updated {output_path} with {len(new_stores)} Etten-Leur stores. Total: {len(combined_stores)}")
